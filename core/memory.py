...
class OllamaEmbeddingFunction:
    ...
    def _embed_single(self, text: str) -> List[float]:
        """Tek bir metin için Ollama'dan embedding al."""
        try:
    ...
    def embed_query(self, text: str) -> List[float]:
        """
        ChromaDB 0.6.x+ ve LangChain uyumluluğu için tek sorgu embedding'i.
        collection.query(query_texts=[...]) bu metodu çağırır.
        """
        return self._embed_single(text)

    def embed_documents(self, texts: List[str]) -> List[List[float]]:
        """
        LangChain uyumluluğu için çoklu belge embedding'i.
        """
...
class MemoryManager:
    ...
    def ingest_documents(self) -> str:
        """
        Belgeleri tara ve vektör veritabanına indeksle
    ...
    def save(
        ...
        # 2. ChromaDB'ye vektör olarak kaydet
    ...
    def _search_history(self, agent: str, query: str) -> str:
        """
        Geçmiş sohbetlerde anlamsal arama
        ...
        try:
            total = self.history_collection.count()
            if total == 0:
                return ""

            n_results = min(MemoryConfig.MAX_HISTORY_RESULTS, total)
    ...
    def _search_documents(self, query: str) -> str:
        ...
        try:
            total = self.docs_collection.count()
            if total == 0:
                return ""

            n_results = min(MemoryConfig.MAX_DOCUMENT_RESULTS, total)
