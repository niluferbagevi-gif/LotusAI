...
def web_chat():
    ...
    if auth_file and RuntimeContext.security_instance:
        try:
            ...
            logger.error(f"Web Auth Hatası: {e}")
    ...
    try:
        ...
        return jsonify({"status": "error", "reply": f"Sistem hatası: {str(e)}"})

@app.route('/webhook', methods=['GET', 'POST'])
def webhook_handler():
    # Webhook kodları buraya (Aynen kopyalanacak)
    if request.method == 'GET':
        ...
        try:
            data = request.json
            if RuntimeContext.messaging_manager:
