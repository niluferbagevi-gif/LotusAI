...
Endpoints:
- / : Ana sayfa
- /sw.js : Service Worker (root scope için)
...
try:
    ...
    logger.warning(f"Upload klasörü oluşturulamadı: {e}")
...
def service_worker() -> Response:
    """
    Service Worker'ı root'tan servis eder.

    Not:
    - sw.js dosyası fiziksel olarak static/ içinde durabilir.
    - Root'tan (/sw.js) servis edince scope tüm siteyi kapsar.
    """
    try:
        resp = make_response(send_from_directory(str(Config.STATIC_DIR), "sw.js"))
        resp.headers["Content-Type"] = "application/javascript; charset=utf-8"
        # SW'nin tüm siteyi (/) kapsamasına izin ver
        resp.headers["Service-Worker-Allowed"] = "/"
        # Geliştirme için cache sorunlarını azaltır
        resp.headers["Cache-Control"] = "no-cache"
        return resp
    except Exception as e:
        logger.error(f"sw.js servis hatası: {e}")
        return make_response("sw.js not found", 404)
...
def toggle_voice_api() -> Response:
    ...
    try:
        ...
        if data and "active" in data:
            RuntimeContext.set_voice_mode(data["active"])
        else:
            RuntimeContext.set_voice_mode(not RuntimeContext.is_voice_mode_active())

        status_msg = "AÇIK" if RuntimeContext.is_voice_mode_active() else "KAPALI"
        logger.info(f"🎙️ Mikrofon modu: {status_msg}")

        return jsonify({"status": "success", "active": RuntimeContext.is_voice_mode_active()})
...
def get_chat_history() -> Response:
    ...
    try:
        ...
        if not RuntimeContext.engine or not RuntimeContext.engine.memory:
            return (
                jsonify(
                    {"status": "error", "message": "Hafıza modülü hazır değil"}
                ),
                503,
            )
...
def web_chat() -> Response:
    ...
    try:
        ...
        if not user_msg and not file_path:
            return jsonify({"status": "error", "reply": "Mesaj içeriği boş"}), 400
        ...
        if is_group_call:
            ...
            return _handle_single_response(
                user_msg, target_agent_req, sec_result, file_path
            )

    except Exception as e:
        logger.error(f"Web chat hatası: {e}")
        return (
            jsonify({"status": "error", "reply": f"Sistem hatası: {str(e)[:100]}"}),
            500,
        )
...
def _handle_file_upload(file) -> Optional[str]:
    ...
    try:
        ...
        # Güvenli ve modern Path birleşimi
...
def _clear_memory() -> Response:
    ...
    return jsonify({"status": "error", "reply": "Hafıza modülü hazır değil"}), 503
...
def _perform_security_check(auth_file) -> tuple:
    ...
    if auth_file and RuntimeContext.security_instance:
        try:
            ...
            if frame is not None:
                frame_present = True
                identified_user = RuntimeContext.security_instance.check_static_frame(
                    frame
                )
    ...
    if identified_user:
        return ("ONAYLI", identified_user, "GÖRSEL_DOGRULAMA_WEB")
    elif frame_present:
...
def _is_group_call(user_msg: str, target_agent: str) -> bool:
    ...
    Returns:
        Group call ise True
    """
    group_triggers = ["millet", "ekip", "herkes", "gençler", "arkadaşlar", "team"]

    return target_agent == "GENEL" and any(t in user_msg.lower() for t in group_triggers)
...
def _handle_team_response(user_msg: str, sec_result: tuple) -> Response:
    ...
    if not RuntimeContext.engine or not RuntimeContext.loop:
        return jsonify({"status": "error", "reply": "Motor hazır değil"}), 503
    ...
    try:
        ...
        return jsonify({"status": "error", "reply": "İşlem zaman aşımına uğradı"}), 504

    except Exception as e:
        logger.error(f"Team response hatası: {e}")
        return jsonify({"status": "error", "reply": "İşlem başarısız"}), 500


def _handle_single_response(
    user_msg: str, target_agent_req: str, sec_result: tuple, file_path: Optional[str]
) -> Response:
    """
    ...
    try:
        future = asyncio.run_coroutine_threadsafe(
            RuntimeContext.engine.get_response(
                final_agent, user_msg, sec_result, file_path=file_path
            ),
            RuntimeContext.loop,
        )
        ...
        if RuntimeContext.is_voice_mode_active():
...
def run_flask() -> None:
    ...
    try:
        ...
        # Config üzerinden dinamik başlatma
        app.run(
