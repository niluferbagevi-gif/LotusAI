"""
LotusAI managers/camera.py - Camera Manager
Sürüm: 2.6.0 (Dinamik Erişim Seviyesi Senkronu & Performans Optimizasyonu)
...
- IR / Boş Frame bypass
- Persistent VideoCapture (Performans artışı)
...
class CameraManager:
    ...
    - CUDA destekli görüntü işleme
    - Dinamik port tarama
    - Kalıcı (Persistent) VideoCapture bağlantısı ile yüksek FPS
    ...
    DEFAULT_FPS = 30
    WARMUP_FRAMES = 5  # WSL ve IR kameralar için ısınma süresi
    JPEG_QUALITY = 80
    ...
    def __init__(self, access_level: Optional[str] = None):
        ...
        self.last_seen_person: Optional[str] = None
    ...
    def start(self) -> bool:
        """
        Kamera donanımını başlat (Kalıcı bağlantı kurar)
        
        Returns:
            Başarılı ise True
        """
        with self.lock:
            if self._active_cap and self._active_cap.isOpened():
                return True

            # Test default port
            cap = self._open_camera_connection(self.camera_index)
            if cap:
            ...
            if active_ports:
                ...
                self._active_cap = self._open_camera_connection(self.camera_index)
            ...
            logger.error("❌ Erişilebilir kamera bulunamadı! (Lütfen bağlantıları kontrol edin)")
            self.status = CameraStatus.DISCONNECTED
            return False

    def _open_camera_connection(self, index: int) -> Optional[cv2.VideoCapture]:
        """
        Belirtilen indexteki kamerayı aç ve ayarlarını yap
        
        Args:
            index: Kamera port index'i
            
        Returns:
            VideoCapture nesnesi veya None
        """
        try:
            ...
            # WSL için MJPG formatını zorla ve buffer'ı minimize et
    ...
    def _test_hardware(self, index: int) -> bool:
        ...
        cap = self._open_camera_connection(index)
        if cap:
            cap.release()
            return True
        return False
    ...
    def _update_camera_info(self) -> None:
        ...
        if self._active_cap and self._active_cap.isOpened():
            self.camera_info = CameraInfo(
                ...
                width=int(self._active_cap.get(cv2.CAP_PROP_FRAME_WIDTH)),
                height=int(self._active_cap.get(cv2.CAP_PROP_FRAME_HEIGHT)),
                fps=self._active_cap.get(cv2.CAP_PROP_FPS),
                backend=self._active_cap.getBackendName(),
    ...
    def get_frame(
        ...
        with self.lock:
            # Kamera açık değilse açmaya çalış
            if not self._active_cap or not self._active_cap.isOpened():
                if not self.start():
                    return None
            ...
            try:
                # Arabelleği temizlemek için fazladan grab at (Latency önler)
                self._active_cap.grab()
                ret, frame = self._active_cap.read()
                
                if not ret or frame is None or frame.size == 0:
                    logger.warning("🚫 Boş görüntü veya zaman aşımı, bağlantı yenileniyor...")
                    self.stop()
                    frame = None
                else:
                    # Flip horizontal
                ...
                self.stop() # Hata durumunda bağlantıyı kapat ki sonraki denemede yeniden açılsın
            
            finally:
                if self.status != CameraStatus.ERROR:
                    self.status = CameraStatus.IDLE
    ...
    def set_resolution(self, width: int, height: int) -> bool:
        ...
        try:
            ...
            # Mevcut kamerayı kapat ki yeni çözünürlük ile açılsın
            self.stop()
    ...
    def stop(self) -> None:
        """
        Kamera servisini durdur (Sistem temizliği için)
        """
        with self.lock:
            if self._active_cap:
