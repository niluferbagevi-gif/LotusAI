...
cudf = None # Global referans

if USE_GPU_CONFIG:
    try:
        import cudf
        HAS_GPU = True
        logger.info("🚀 AccountingManager: GPU (cuDF) hızlandırma aktif.")
    except ImportError:
        logger.info("ℹ️ AccountingManager: Config GPU açık ancak cuDF bulunamadı. CPU (Pandas) kullanılacak.")
else:
    logger.info("ℹ️ Muhasebe işlemleri CPU modunda (Config ayarı).")
...
class AccountingManager:
    ...
    Yetenekler:
    - GPU Hızlandırma: cuDF desteği ile büyük veri setlerinde yüksek performans (Config kontrollü).
    ...
    def __init__(self):
        ...
        logger.info(f"✅ Muhasebe Yöneticisi aktif. Mod: {status}")
