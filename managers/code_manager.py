"""
LotusAI managers/code_manager.py - Code Manager
...
- İşletim sistemi path uyumluluğu (Shlex POSIX/NT Düzeltmesi)
- GPU bilgisi
- Otomatik yedekleme
- Kod arama
...
class CodeManager:
    ...
    - Erişim seviyesi: restricted (sadece okuma), sandbox (okuma+güvenli yazma), full (tüm yetkiler)
    """
    ...
    ALLOWED_COMMANDS_FULL = {
        ...
        "find", "grep", "nvidia-smi", "ps", "top", "htop", "ollama"
    }
    ...
    # NOT: Burada hem tam isim hem glob desenleri olabilir (örn. "*.pyc")
    EXCLUDE_FILES = {
    ...
    def __init__(
        ...
        self.access_level = (access_level or Config.ACCESS_LEVEL or AccessLevel.SANDBOX).lower()
        ...
        # Sandbox yazma dizini (mutlak) — Config.SANDBOX_DIR genelde WORK_DIR/temp
        try:
            self.temp_dir = Path(Config.SANDBOX_DIR).resolve()
        except Exception:
            self.temp_dir = (self.root_dir / "temp").resolve()

        # Yedekleme dizini Config.DATA_DIR altında
        self.backup_dir = Config.DATA_DIR / "code_backups"
        ...
        logger.info(f"✅ CodeManager aktif (Root: {self.root_dir}, Temp: {self.temp_dir}, Erişim: {self.access_level})")
    ...
    def _is_within_temp(self, path: Path) -> bool:
        """
        Sandbox modunda yazmanın sadece temp dizinine olmasını garanti eder.

        Args:
            path: Mutlak/resolve edilmiş path

        Returns:
            temp altında ise True
        """
        try:
            return path.resolve().is_relative_to(self.temp_dir)
        except Exception:
            return False
    ...
    def _is_excluded_file(self, filename: str) -> bool:
        """
        EXCLUDE_FILES setindeki hem tam isim hem glob desenlerini destekler.
        """
        ...
        with self.lock:
            try:
                ...
                for path in search_func(pattern):
                    ...
                    if path.is_file():
                        if self._is_excluded_file(path.name):
                            continue
                ...
                return "\n".join(sorted(files)) if files else "🔍 Eşleşen dosya yok"
    ...
    def save_file(self, filename: str, content: str) -> str:
        ...
        with self.lock:
            try:
                ...
                # Security check: proje dışına yazma yasak
                if not self._is_safe_path(target_path):
                    return "[GÜVENLİK]: Sandbox dışına yazma yasak"

                # Sandbox: sadece temp dizinine yaz
                if self.access_level == AccessLevel.SANDBOX:
                    if not self._is_within_temp(target_path):
                        return (
                            f"🚫 [GÜVENLİK]: Sandbox modundasınız.\n"
                            f"Dosyayı sadece '{self.temp_dir}' klasörüne kaydedebilirsiniz.\n"
                            f"Hedeflenen yol: {filename}"
                        )
                ...
                logger.info(f"💾 Dosya kaydedildi: {target_path}")
    ...
    def delete_file(self, filename: str) -> str:
        ...
        with self.lock:
            try:
                ...
                logger.warning(f"🗑️ Dosya silindi: {target_path}")
    ...
    def get_file_info(self, filename: str) -> str:
        ...
        try:
            ...
            if target_path.suffix in {".py", ".txt", ".md"}:
                try:
                    lines = len(target_path.read_text(encoding="utf-8", errors="ignore").splitlines())
                except Exception:
                    pass
        ...
        with self.lock:
            ...
            try:
                for path in self.root_dir.rglob(file_ext):
                    ...
                    if path.is_file() and self._is_excluded_file(path.name):
                        continue
                ...
                if results:
                    return f"🔍 '{query}' bulundu ({len(results)} dosya):\n" + "\n".join(results)
    ...
    def run_terminal(self, command: str, timeout: int = DEFAULT_TIMEOUT) -> str:
        """
        ...
        with self.lock:
            try:
                ...
                use_shell = (
                    is_windows
                    and base_cmd in {"dir", "echo", "type", "mkdir", "date", "tree"}
                )
                ...
                logger.info(f"💻 Terminal ({self.access_level}): {command[:80]}")

                return f"--- TERMİNAL ---\n{output.strip()}" if output.strip() else "✅ İşlem tamamlandı"

            except FileNotFoundError:
                return f"❌ Komut bulunamadı: '{base_cmd}'. Sisteminizde yüklü olmayabilir."
...
# İyileştirmeler (2.6.0 içi patch):
# - Sandbox yazma kontrolü: string startswith yerine Path.is_relative_to + resolve
# - EXCLUDE_FILES glob desenleri: fnmatch ile destek
# - Mesajlarda gerçek temp dizini gösterimi
# """
