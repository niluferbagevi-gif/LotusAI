"""
LotusAI managers/finance.py - Finance Manager
Sürüm: 2.6.0 (Dinamik Erişim Seviyesi Senkronu & Optimizasyon)
Açıklama: Finans, borsa ve analiz yönetimi

Özellikler:
- CCXT borsa entegrasyonu (API Anahtarı Destekli)
- Teknik analiz (RSI, EMA, MACD)
- Optimize edilmiş hesaplamalar (VRAM Memory Leak Düzeltildi)
- Grafik oluşturma
- Piyasa özeti
- Cache sistemi
    ...
    try:
        ...
        if torch.cuda.is_available():
            ...
            try:
                ...
                logger.info(f"🚀 Finance GPU tespiti yapıldı: {gpu_name}")
            except Exception:
                logger.info("🚀 Finance GPU tespiti yapıldı")
        else:
            logger.info("ℹ️ CUDA yok, CPU kullanılacak")
    except ImportError:
        logger.info("ℹ️ PyTorch yok, GPU tespiti atlandı")
...
class FinanceManager:
    ...
    def _init_exchange(self) -> None:
        """Borsa bağlantısı başlat (API Key Entegrasyonu İle)"""
        try:
            exchange_config = {
            ...
            # API Anahtarları varsa ekle (Kişisel bakiye işlemleri için)
            if getattr(Config, "BINANCE_API_KEY", None) and getattr(Config, "BINANCE_API_SECRET", None):
                ...
                logger.info("🔑 Binance API anahtarları sisteme yüklendi")
    ...
    def get_balance(self) -> str:
        ...
        # Muhasebe modülü yoksa, Binance API'den çekmeyi deneyebilir
        if self.exchange and self.exchange.has['fetchBalance'] and getattr(Config, "BINANCE_API_KEY", None):
            try:
                balance = self.exchange.fetch_balance()
                usdt_free = balance.get('USDT', {}).get('free', 0.0)
                return f"{usdt_free:,.2f} USDT (Borsa Bakiyesi)"
            except Exception as e:
                logger.warning(f"Borsa bakiye çekim hatası: {e}")

        return "12,450.00 TRY (Demo Kasa)"
    ...
    def _calculate_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        ta kütüphanesi CPU üzerinde C tabanlı Pandas fonksiyonlarıyla son derece optimize çalışır.
        (VRAM Sızıntısını önlemek için GPU tensor kopyalamaları kaldırılmıştır)
        """
        try:
            # Calculate indicators (CPU - highly optimized)
    ...
    def _format_analysis_report(
        ...
        report_lines = [
            f"📊 {analysis.symbol} TEKNİK ANALİZ ({analysis.timeframe})",
    ...
    def _generate_chart(
        ...
        """
        Grafik oluştur ve Static klasörüne kaydet.
        """
        try:
    ...
    def _open_chart(self, path: Path) -> None:
        """Grafiği aç (debug modunda pop-up)"""
        try:
