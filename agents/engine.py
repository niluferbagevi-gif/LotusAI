"""
LotusAI agents/engine.py - Agent Engine - Multi-Agent Koordinasyon Motoru
...
Güncelleme: CODING_MODEL (Sidar) desteği, ajan bazlı Ollama model seçimi, görsel analiz düzeltmeleri.
"""
...
class EngineConfig:
    ...
    VISUAL_TRIGGERS = ["fatura", "fiş", "dekont", "hesap", "oku", "işle",
                       "ne yazıyor", "analiz et", "göster", "bak", "bu ne"]
...
class AgentEngine:
    ...
    def __init__(self, memory_manager: Any, tools_dict: Dict[str, Any], access_level: Optional[str] = None):
        ...
        # Erişim seviyesini Config'den veya parametreden al
        ...
        logger.info(f"✅ AgentEngine hazır (Device: {self.device.upper()}, Erişim: {self.access_level.upper()})")

    def _initialize_agents(self) -> None:
        """Tüm agent'ları başlat (Erişim seviyesini ilet)"""
        ...
        if "sidar_special" in self.tools:
            ...
            sidar_tools = {
                k: self.tools.get(k)
                for k in ['code', 'system', 'security', 'memory', 'github']
            }
    ...
    def _resolve_ollama_model(self, agent: str, user_text: str = "", has_image: bool = False) -> str:
        """
        Ollama için ajan ve içeriğe göre kullanılacak modeli belirle.

        Kural:
        1- Görsel varsa veya tetiklendiyse SİSTEM ÇÖKMEMESİ için her zaman VISION_MODEL
        2- Ajan SIDAR ise CODING_MODEL (qwen2.5-coder:7b)
        3- Diğer tüm durumlar için TEXT_MODEL (gemma2:9b)

        Args:
            agent: Agent adı
            user_text: Kullanıcı metni
            has_image: Görsel data iletildi mi?
        ...
        # 1. Kural: Görsel varsa veya analiz isteniyorsa kesinlikle VISION_MODEL seçilmeli.
        # (Örn: qwen2coder'a resim atılırsa Ollama HTTP 400 hatası verir ve çöker)
        if has_image or any(t in clean_text for t in EngineConfig.VISUAL_TRIGGERS):
            logger.debug(f"{agent} → VISION_MODEL: {Config.VISION_MODEL}")
            return Config.VISION_MODEL

        # 2. Kural: SIDAR her zaman coding modeli kullanır
        if agent_upper == "SIDAR":
            logger.debug(f"SIDAR → CODING_MODEL: {Config.CODING_MODEL}")
            return Config.CODING_MODEL

        # 3. Kural: Varsayılan metin modeli
    ...
    def _load_file_for_gemini(self, file_path: Union[str, Path]) -> Tuple[Optional[Dict[str, Any]], Optional[str]]:
        """Dosyayı MultiModal (Gemini/Ollama) formatına hazırla"""
        path = Path(file_path)
    ...
    def _extract_json_from_text(self, text: str) -> Optional[Dict]:
        ...
        try:
            # Markdown parse hatasını engellemek için kodlanmış string yapısı
    ...
    def _build_core_prompt(
        ...
        active_model_info = ""
        if Config.AI_PROVIDER == "ollama":
            active_model = self._resolve_ollama_model(agent_name, user_text, has_image)
        ...
        access_display = {
            AccessLevel.RESTRICTED: "🔒 Kısıtlı (Restricted)",
            AccessLevel.SANDBOX: "📦 Sandbox (Güvenli Alan)",
            AccessLevel.FULL: "⚡ Tam Erişim (Full)"
        }.get(self.access_level, self.access_level)

        sections.extend([
            "",
            "### ERİŞİM SEVİYESİ PROTOKOLÜ ###",
            f"MEVCUT YETKİ SEVİYEN: {access_display}",
            "BU SEVİYENİN KURALLARINA KESİNLİKLE UYMALISIN:",
        ])

        if self.access_level == AccessLevel.RESTRICTED:
            sections.extend([
                "- Sadece bilgi ver, analiz yap ve öneride bulun.",
                "- ASLA dosya yazma, silme veya terminal komutu önerme/çalıştırma.",
                "- Kullanıcı yazmanı isterse, 'Kısıtlı modda olduğum için bunu yapamam' de."
            ])
        elif self.access_level == AccessLevel.SANDBOX:
            sections.extend([
                "- Dosya yazma işlemlerin SADECE '/temp' klasörü ile sınırlıdır.",
                "- Sistem dosyalarına dokunamazsın.",
                "- Kod yazarken güvenli bir şekilde '/temp' altına kaydettiğini belirt."
            ])
        elif self.access_level == AccessLevel.FULL:
            sections.extend([
                "- Tüm sistem yetkilerine sahipsin.",
                "- Dosya yazma ve sistem komutları serbesttir.",
                "- Ancak yine de kritik işlemlerde kullanıcı onayı iste."
            ])
    ...
    async def _handle_visual_tasks(
        ...
        """Görsel analiz ve belge işleme (Ollama Vision Uyumlu)"""
        ...
        # Eğer Görüntü başarıyla alındıysa AI servisine gönder
        if gemini_file_part:
            analysis_prompt = (
            ...
            # Dinamik Provider Yönlendirmesi
    ...
    async def _handle_github_tasks(self, clean_input: str, agent_name: str) -> Optional[str]:
        """GitHub görevlerini işle (Null Reference Bug Düzeltildi)"""
        if agent_name not in ["SIDAR", "ATLAS"]:
            return None
            
        # tools['github'] None olabilir, safe check:
        if not self.tools.get("github"):
            return None
        ...
        if "oku" in clean_input or "incele" in clean_input:
            ...
            if file_match:
                ...
                # Markdown parse hatasını önlemek için güvenli kod blokları
                return (
    ...
    async def get_response(
        ...
        if status not in ["ONAYLI", "SES_ONAYLI"]:
            if sub_status == "KAMERA_YOK":
                return {
                    "agent": "KERBEROS",
                    "content": "Güvenlik protokolü gereği sizi tanımam gerekiyor. Lütfen kameranızı aktive edin veya sesli doğrulama yapın."
                }
            
            # Doğrulanmamış kullanıcı - KERBEROS yanıt verir
            sys_prompt = self._build_core_prompt("KERBEROS", clean_input, sec_result)
            if Config.AI_PROVIDER == "gemini" and GENAI_AVAILABLE:
                return await self._query_gemini("KERBEROS", sys_prompt, [], user_text)
            else:
                return await self._query_ollama("KERBEROS", sys_prompt, [], user_text)
        ...
        if status in ["ONAYLI", "SES_ONAYLI"] and len(clean_input.split()) <= 3:
            if any(word in clean_input for word in EngineConfig.WELCOME_KEYWORDS):
                ...
                return {
                    "agent": agent_name,
                    "content": f"Hoş geldiniz {name}. LotusAI {self.device.upper()} destekli sistemleriyle aktif. Size nasıl yardımcı olabilirim?"
                }
        ...
        # 3. GÖRSEL/DOSYA İŞLEME (Ollama uyumlu hale getirildi)
        ...
        sys_prompt = self._build_core_prompt(
            ...
            sec_result, 
            op_result,
            has_image=(gemini_file_part is not None)
        )
    ...
    async def _query_gemini(
        ...
        for attempt in range(EngineConfig.MAX_RETRIES):
            try:
                ...
                logger.warning(f"Gemini deneme {attempt + 1}/{EngineConfig.MAX_RETRIES} başarısız: {e}")

                if attempt == EngineConfig.MAX_RETRIES - 1:
                    logger.error(f"Gemini kritik hatası ({agent}): {e}")
                    return {
                        "agent": agent,
                        "content": "Şu an merkezi sinir sistemime (Gemini) ulaşamıyorum. Lütfen kısa süre sonra tekrar deneyin."
                    }
    ...
    async def _query_ollama(
        ...
        """
        Ollama API'ye istek gönder (Görsel Analiz Uyumlu).
        """
        has_image = (image_data is not None)
        selected_model = self._resolve_ollama_model(agent, user_text, has_image)

        # Kullanıcı mesajını oluştur (Eğer resim varsa Base64'e çevir ve ekle)
        user_message = {"role": "user", "content": user_text}
        if has_image and "data" in image_data:
            try:
                b64_image = base64.b64encode(image_data["data"]).decode("utf-8")
                user_message["images"] = [b64_image]
            except Exception as e:
                logger.error(f"Ollama Base64 dönüşüm hatası: {e}")
                    ...
                    async with session.post(
                        ...
                        if response.status == 200:
                            ...
                            return {
                                "agent": agent,
                                "content": (
                                    f"⚠️ **Ollama Hatası (404):** İstenen model \x60{selected_model}\x60 bulunamadı.\n"
                                    f"Lütfen terminalde şu komutu çalıştırın:\n"
                                    f"\x60ollama pull {selected_model}\x60"
                                )
                            }

                        else:
                            body = await response.text()
                            logger.error(f"Ollama HTTP hatası: {response.status} | {body[:200]}")

            except asyncio.TimeoutError:
                logger.error(f"Ollama timeout (Agent: {agent}, Model: {selected_model}, Deneme: {attempt + 1}/2)")
...
#         # Erişim seviyesi bilgisi ekle (Prompt Mühendisliği)
#         access_display = {
#             AccessLevel.RESTRICTED: "🔒 Kısıtlı (Restricted)",
#             AccessLevel.SANDBOX: "📦 Sandbox (Güvenli Alan)",
#             AccessLevel.FULL: "⚡ Tam Erişim (Full)"
...
#             # Doğrulanmamış kullanıcı — sadece KERBEROS yanıt verir, akış durdurulur
#             sys_prompt = self._build_core_prompt("KERBEROS", clean_input, sec_result)
#             return await self._query_gemini(
#                 "KERBEROS", sys_prompt, [], user_text
#             ) if Config.AI_PROVIDER == "gemini" else await self._query_ollama(
#                 "KERBEROS", sys_prompt, [], user_text
#             )
